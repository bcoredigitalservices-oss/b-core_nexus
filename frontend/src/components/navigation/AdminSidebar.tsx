...
export default function AdminSidebar({ isOpen, onClose, onCollapse, isMobile }: AdminSidebarProps) {
  ...
  const navGroups: NavGroup[] = [
    {
      ...
      items: [
        ...
        { label: 'Workspaces', path: '/workspace', icon: <Briefcase size={17} /> }
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Customers', path: '/customers', icon: <UserCheck size={17} />, readOnly: true }
      ]
    },
    {
      title: 'System Integrations',
      items: [
        { label: 'Email Setup', path: '/email-setup', icon: <Mail size={17} /> },
        { label: 'Automation Setup', path: '/automation', icon: <Zap size={17} /> },
        { label: 'Tax Rules & Services', path: '/taxes', icon: <Percent size={17} />, readOnly: true },
        { label: 'Communication', path: '/communication', icon: <MessageSquare size={17} />, subtext: 'Chat Rules & Configs' }
      ]
    },
    {
      title: 'Control & Security',
      items: [
        { label: 'Organisation', path: '/settings/org', icon: <Globe size={17} />, readOnly: true },
        { label: 'Security Logs', path: '/events', icon: <Shield size={17} /> },
        { label: 'Event Engine', path: '/event-engine', icon: <Terminal size={17} /> },
        { label: 'System Settings', path: '/settings/config', icon: <Settings size={17} />, subtext: 'Themes, Fonts, Layouts' }
      ]
    }
  ];
  ...
  return (
    <>
      ...
      >
          ...
          {!effectiveCollapsed ? (
            <div className="flex items-center gap-2.5 overflow-hidden whitespace-nowrap">
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="text-[10px] text-text-muted uppercase tracking-widest truncate max-w-[140px]" title={systemSettings?.organization_name || 'Tier 0 Root Admin'}>
            ...
            <div key={groupIdx} className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-col gap-0.5">
                {group.items.map((item, index) => (
                  ...
                  >
                    <span className="flex items-center justify-center opacity-85 shrink-0">{item.icon}</span>
                    {!effectiveCollapsed && (
                        ...
                        <div className="flex items-center justify-between w-full">
                          ...
                          {item.label === 'Users' && (
                            ...
                            <button
                              ...
                              className="bg-[#00A0DF] bg-opacity-10 border border-[#00A0DF] border-opacity-30 rounded text-[#00A0DF] p-0.5 flex items-center cursor-pointer transition-all duration-150 hover:bg-opacity-25 hover:border-opacity-50 shrink-0"
        ...
        {/* Footer / User Profile */}
        <div className={`p-4 border-t border-color flex flex-col gap-2.5 shrink-0 ${effectiveCollapsed ? 'items-center' : ''}`}>
          {currentUser && (
            !effectiveCollapsed ? (
