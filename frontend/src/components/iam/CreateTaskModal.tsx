      ...
      <div className="glass-panel w-full max-w-[500px] bg-card border border-color rounded-2xl p-6 shadow-xl relative animate-[float-in_0.25s_ease] flex flex-col gap-5">
        ...
        <div className="border-b border-color pb-3">
          ...
          <span className="text-[10px] text-text-muted mt-1 block">
            {task ? 'Modify existing task details and tracking' : 'Create and assign operational task logs'}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] text-text-muted font-bold uppercase tracking-wider">Task Title *</label>
              <input 
                type="text"
                required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-text-muted font-bold uppercase tracking-wider">Assignee *</label>
                {preselectedUserId ? (
                  ...
                  >
                    ...
                    {operators.map((op) => (
                      <option key={op.id} value={op.id}>
                        {op.first_name ? `${op.first_name} ${op.last_name || ''} (${op.email})` : op.email}
                      </option>
                    ))}
                  </select>
                )}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-text-muted font-bold uppercase tracking-wider">Due Date</label>
                <div className="relative flex items-center">
                  <Calendar size={12} className="absolute left-3 text-text-muted pointer-events-none" />
                  <input 
                    type="date"
                    value={dueDate}
