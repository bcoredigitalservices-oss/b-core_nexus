...
export default function MyProfileSettings() {
  ...
  // ── MFA State ────────────────────────────────────────────────────────
  const [mfaStatus, setMfaStatus] = useState<boolean>(currentUser?.is_totp_enabled ?? false);
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setVerifySuccess('✓ Security PIN configured! Your MFA authenticator is now linked. Next time you log in, you will be asked for your 6-digit security code.');
      setMfaStatus(true);
      setStep('idle');
      setSetupUri(null);
      setSecret(null);
  ...
  const initials = [currentUser?.first_name?.[0], currentUser?.last_name?.[0]].filter(Boolean).join('').toUpperCase() || '?';
    ...
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] p-6 md:p-10">
      <div className="max-w-[820px] mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-extrabold tracking-tight text-[var(--text-main)]">My Profile</h1>
          <p className="text-sm text-[var(--text-muted)]">Manage your account settings and security preferences.</p>
        </div>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6 flex items-center gap-5 shadow-sm">
          ...
          <div className="flex flex-col gap-1 min-w-0">
            <span className="text-lg font-bold text-[var(--text-main)] truncate">{fullName}</span>
            <span className="text-sm text-[var(--text-muted)] truncate">{currentUser?.email}</span>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {(currentUser?.functional_roles ?? []).length === 0 && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider bg-white/5 border border-white/10 text-[var(--text-muted)]">
                  {currentUser?.permissions?.includes('*:*') ? 'Administrator' : 'User'}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="flex items-center gap-3 px-6 py-5 border-b border-[var(--border-color)]">
            <div className="w-9 h-9 rounded-xl bg-[rgba(157,78,221,0.12)] border border-[rgba(157,78,221,0.2)] flex items-center justify-center">
              <Shield size={17} className="text-[var(--accent-purple,#9d4edd)]" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-[var(--text-main)]">Two-Factor Authentication (2FA)</h2>
              <p className="text-xs text-[var(--text-muted)]">Add an extra layer of security to your account using an authenticator app.</p>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col gap-5">
            ...
            {mfaStatus ? (
              <div className="flex items-center gap-3 p-4 bg-[rgba(0,245,160,0.06)] border border-[rgba(0,245,160,0.2)] rounded-xl">
                <ShieldCheck size={18} className="text-[var(--accent-green)] shrink-0" />
                <div>
                  <div className="text-sm font-semibold text-[var(--text-main)]">Security PIN is active</div>
                  <div className="text-xs text-[var(--text-muted)] mt-0.5">Your account is protected with a time-based one-time password (TOTP). You will be prompted for a 6-digit code on every login.</div>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-4 bg-[rgba(255,51,102,0.05)] border border-[rgba(255,51,102,0.2)] rounded-xl">
                ...
                <div>
                  <div className="text-sm font-semibold text-[var(--text-main)]">Security PIN is not configured</div>
                  <div className="text-xs text-[var(--text-muted)] mt-0.5">Without 2FA, your account is protected only by your password. Set up a security PIN to significantly improve your account security.</div>
                </div>
              </div>
            )}
            ...
            {verifySuccess && (
              <div className="flex items-start gap-3 p-4 bg-[rgba(0,245,160,0.06)] border border-[rgba(0,245,160,0.2)] rounded-xl">
                <CheckCircle2 size={16} className="text-[var(--accent-green)] mt-0.5 shrink-0" />
                <p className="text-sm text-[var(--text-main)]">{verifySuccess}</p>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2 text-sm text-[var(--text-muted)]">
                  <div className="flex items-start gap-2">
                    <span className="text-[var(--accent-primary)] font-bold mt-0.5">1.</span>
                    <span>Install an authenticator app on your phone (e.g. <strong>Google Authenticator</strong>, <strong>Microsoft Authenticator</strong>, or <strong>Authy</strong>).</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-[var(--accent-primary)] font-bold mt-0.5">2.</span>
                    <span>Click <strong>"Set Up Security PIN"</strong> below to generate your unique QR code.</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-[var(--accent-primary)] font-bold mt-0.5">3.</span>
                    <span>Scan the QR code with your authenticator app, then enter the 6-digit code to confirm.</span>
                  </div>
                </div>
                ...
                >
                  {mfaLoading ? <Loader2 size={15} className="animate-spin" /> : <QrCode size={15} />}
              ...
              <div className="flex flex-col gap-6">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                  ...
                  <div className="flex flex-col items-center gap-3 bg-white p-4 rounded-2xl shadow-sm shrink-0">
                    ...
                    <span className="text-[10px] text-gray-400 font-mono text-center">Scan with your authenticator app</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-4 flex-1">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <Smartphone size={15} className="text-[var(--accent-primary)]" />
                        <span className="text-sm font-semibold text-[var(--text-main)]">Scan the QR code</span>
                      </div>
                      <p className="text-xs text-[var(--text-muted)] leading-relaxed">
                        Open your authenticator app and scan the QR code on the left. If you can't scan, manually enter the secret key below.
                      </p>
                    </div>
                    ...
                    {secret && (
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-2 bg-white/5 border border-[var(--border-color)] rounded-lg px-3 py-2">
                          ...
                          >
                            {copied ? <CheckCircle2 size={13} className="text-[var(--accent-green)]" /> : <Copy size={13} />}
                          </button>
                        </div>
                      </div>
                    )}
            ...
            {/* Step: Verify code */}
            {step === 'verify' && (
              <form onSubmit={handleVerifyViaLogin} className="flex flex-col gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Key size={15} className="text-[var(--accent-primary)]" />
                    <span className="text-sm font-semibold text-[var(--text-main)]">Enter the 6-digit code</span>
                  </div>
                  <p className="text-xs text-[var(--text-muted)]">
                    Open your authenticator app and enter the current 6-digit code for B-Core Nexus to confirm the setup.
                  </p>
                </div>
            ...
            {step === 'idle' && mfaStatus && (
              <div className="flex flex-col gap-3">
                <p className="text-xs text-[var(--text-muted)]">
                  Your security PIN is already active. If you've lost access to your authenticator app or want to reset it, you can generate a new QR code below. This will replace your current setup.
                </p>
                <button
                  id="reset-mfa-btn"
                  onClick={handleStartSetup}
                ...
                >
                  {mfaLoading ? <Loader2 size={14} className="animate-spin" /> : <QrCode size={14} />}
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center gap-3 px-6 py-5 border-b border-[var(--border-color)]">
            ...
            <div>
              <h2 className="text-sm font-bold text-[var(--text-main)]">Account Information</h2>
              <p className="text-xs text-[var(--text-muted)]">Your identity details as configured by your administrator.</p>
            </div>
          </div>
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            {[
              ...
              { label: 'Account Status', value: currentUser?.is_active ? 'Active' : 'Inactive' },
            ].map(({ label, value }) => (
              <div key={label} className="flex flex-col gap-1">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{label}</span>
                <span className="text-sm text-[var(--text-main)] font-medium">{value}</span>
              </div>
            ))}
          </div>
        </div>
