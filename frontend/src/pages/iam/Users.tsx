...
export default function Users() {
  ...
  // Sync tab with search params changes
  useEffect(() => {
  ...
  // Fetch directory, roles & permissions data
  const loadDirectoryData = async () => {
    try {
      setLoading(true);
      setErrorMsg('');

      const [usersData, workspacesData, departmentsData, rolesData] = await Promise.all([
  ...
  // Populate inputs when selected role changes
  useEffect(() => {
  ...
  // Save changes to existing role (metadata)
  ...
  const getClearanceBadge = (permissions: string[] = [], functionalRoles: string[] = []) => {
    ...
    if (functionalRoles && functionalRoles.length > 0) {
      const formatted = functionalRoles[0]
        ...
        .join(' ');
      return { label: formatted, color: 'var(--text-muted)', bg: 'rgba(142, 139, 130, 0.06)', border: '1px solid rgba(142, 139, 130, 0.15)' };
    }
    ...
    <div className="flex flex-col gap-8 w-full max-w-[1250px] mx-auto p-4 relative animate-[fadeIn_0.3s_ease]">
      ...
      <div className="flex border-b border-color gap-6">
        <button
          ...
          className={`pb-3 text-sm font-bold tracking-wide uppercase transition-all relative cursor-pointer ${
            activeTab === 'users' ? 'text-text-main font-bold' : 'text-text-muted hover:text-text-main'
          }`}
        >
          Operator Directory
          {activeTab === 'users' && (
            <div className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-accent-primary" />
          )}
        </button>
        <button
          onClick={() => handleTabChange('roles')}
          className={`pb-3 text-sm font-bold tracking-wide uppercase transition-all relative cursor-pointer ${
            activeTab === 'roles' ? 'text-text-main font-bold' : 'text-text-muted hover:text-text-main'
          }`}
        >
          Authority Roles
          {activeTab === 'roles' && (
      ...
      {activeTab === 'users' ? (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                <span className="font-semibold text-text-main text-[0.88rem]">
                                  {user.full_name || (user.first_name ? `${user.first_name} ${user.last_name || ''}` : <em className="opacity-50 font-normal">Unconfigured Profile</em>)}
                                </span>
                          ...
                          <td className="py-4 px-6 text-right flex justify-end items-center gap-2">
                            ...
                            {user.is_active ? (
                              <button
                                ...
                                disabled={!canToggleStatus || updatingUserId === user.id}
                                className="py-1.5 px-3 bg-red-500/10 border border-red-500/25 text-red-400 text-[0.75rem] rounded-lg cursor-pointer hover:bg-red-500/20 hover:border-red-500/45 disabled:opacity-50 disabled:cursor-not-allowed font-semibold inline-flex items-center gap-1.5 transition shadow-sm h-[32px]"
                                title={!canToggleStatus ? "Insufficient permissions" : undefined}
                              >
                                <Lock size={12} />
                                {updatingUserId === user.id ? 'Suspending...' : 'Deactivate'}
                              </button>
                            ) : (
                              <button
                                onClick={() => handleToggleStatus(user.id, false)}
                                disabled={!canToggleStatus || updatingUserId === user.id}
                                className="py-1.5 px-3 bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 text-[0.75rem] rounded-lg cursor-pointer hover:bg-emerald-500/20 hover:border-emerald-500/45 disabled:opacity-50 disabled:cursor-not-allowed font-semibold inline-flex items-center gap-1.5 transition shadow-sm h-[32px]"
                                title={!canToggleStatus ? "Insufficient permissions" : undefined}
                              >
                                <Unlock size={12} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-[fadeIn_0.15s_ease]">
          ...
          {/* Right Side: Role Settings & Editor Panel */}
            ...
            {selectedRole && !isCreatingRole && (
              <div className="glass-panel p-6 bg-card border border-color rounded-xl shadow-sm flex flex-col gap-6 animate-[fadeIn_0.15s_ease]">
                ...
                <div className="flex justify-between items-center border-b border-color pb-3 flex-wrap gap-4">
                  ...
                  >
                    {roleSubmitting ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                    Save Role Settings
                  </button>
                </div>
