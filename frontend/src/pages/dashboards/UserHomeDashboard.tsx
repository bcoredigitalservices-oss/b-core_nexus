...
import {
  ...
  CalendarDays, Loader2, CheckSquare, Square, Zap,
  Sun, Sunset, Moon, Bell, DollarSign, FileText,
  CreditCard, Building, Percent, BarChart2, PieChart as PieIcon,
  Briefcase, Receipt, LineChart, Package, ShoppingCart,
  Monitor, Users, Tag, LifeBuoy, Megaphone, Globe,
  UserCheck, Wallet, Calendar, Award, Plane, MessageSquare,
  Settings, Cog, Cpu, FolderKanban, Truck, Wrench, Map,
  Shield, Layers, LayoutGrid, ArrowRight, Server, Activity,
  Lock, Building2, Boxes, Box, MessagesSquare, Factory, BadgeCheck,
  UserPlus, Headphones, PiggyBank, Compass, TrendingUp, Mail
} from 'lucide-react';
import {
...
import CreateTaskModal from '../../components/iam/CreateTaskModal';
...
const WORKSPACE_MAP: Record<string, { label: string; route: string; icon: React.ReactNode; color: string }> = {
  accounting:      { label: 'Accounting',      route: '/workspaces/finance/accounting',    icon: <DollarSign size={20} />,    color: '#00f5a0' },
  invoicing:       { label: 'Invoicing',        route: '/workspaces/finance/invoicing',     icon: <FileText size={20} />,      color: '#00f5a0' },
  payments:        { label: 'Payments',         route: '/workspaces/finance/payments',      icon: <CreditCard size={20} />,    color: '#00f5a0' },
  banking:         { label: 'Banking',          route: '/workspaces/finance/banking',       icon: <Building size={20} />,      color: '#00f5a0' },
  taxes:           { label: 'Taxes',            route: '/workspaces/finance/taxes',         icon: <Percent size={20} />,       color: '#00f5a0' },
  reports:         { label: 'Reports',          route: '/workspaces/finance/reports',       icon: <BarChart2 size={20} />,     color: '#00f5a0' },
  budget:          { label: 'Budget',           route: '/workspaces/finance/budgets',       icon: <PieIcon size={20} />,       color: '#00f5a0' },
  shares:          { label: 'Shares',           route: '/workspaces/finance/shares',        icon: <Briefcase size={20} />,     color: '#00f5a0' },
  expenses:        { label: 'Expenses',         route: '/workspaces/finance/expenses',      icon: <Receipt size={20} />,       color: '#00f5a0' },
  assets:          { label: 'Assets',           route: '/workspaces/inventory',             icon: <Briefcase size={20} />,     color: '#ffb703' },
  products:        { label: 'Products',         route: '/workspaces/inventory/products',    icon: <Package size={20} />,       color: '#ffb703' },
  warehouse:       { label: 'Warehouse',        route: '/workspaces/inventory/warehouse',   icon: <Building size={20} />,      color: '#ffb703' },
  stock:           { label: 'Stock',            route: '/workspaces/inventory/stock',       icon: <ClipboardList size={20} />, color: '#ffb703' },
  buying:          { label: 'Buying',           route: '/workspaces/inventory/buying',      icon: <ShoppingCart size={20} />,  color: '#ffb703' },
  pos:             { label: 'POS',              route: '/workspace/crm/pos',                icon: <Monitor size={20} />,       color: '#00f2fe' },
  crm:             { label: 'CRM',              route: '/workspace/crm',                    icon: <Users size={20} />,         color: '#00f2fe' },
  sales:           { label: 'Sales',            route: '/workspace/crm/sales-orders',       icon: <Tag size={20} />,           color: '#00f2fe' },
  support:         { label: 'Support',          route: '/workspace/crm/support',            icon: <LifeBuoy size={20} />,      color: '#00f2fe' },
  marketing:       { label: 'Marketing',        route: '/workspace/crm/marketing',          icon: <Megaphone size={20} />,     color: '#00f2fe' },
  website:         { label: 'Website',          route: '/workspace/crm/website',            icon: <Globe size={20} />,         color: '#00f2fe' },
  hr:              { label: 'HR',               route: '/workspace/hr',                     icon: <UserCheck size={20} />,     color: '#f472b6' },
  payroll:         { label: 'Payroll',          route: '/workspace/hr/payroll',             icon: <Wallet size={20} />,        color: '#f472b6' },
  attendance:      { label: 'Attendance',       route: '/workspace/hr/attendance',          icon: <Calendar size={20} />,      color: '#f472b6' },
  recruitment:     { label: 'Recruitment',      route: '/workspace/hr/recruitment',         icon: <Award size={20} />,         color: '#f472b6' },
  performance:     { label: 'Performance',      route: '/workspace/hr/performance',         icon: <LineChart size={20} />,     color: '#f472b6' },
  leaves:          { label: 'Leaves',           route: '/workspace/hr/leaves',              icon: <Plane size={20} />,         color: '#f472b6' },
  employee_groups: { label: 'Employee Groups',  route: '/workspace/hr/employee-groups',     icon: <Users size={20} />,         color: '#f472b6' },
  manufacturing:   { label: 'Manufacturing',    route: '/workspace/operations',             icon: <Cpu size={20} />,           color: '#c084fc' },
  projects:        { label: 'Projects',         route: '/workspace/operations/projects',    icon: <FolderKanban size={20} />,  color: '#c084fc' },
  logistics:       { label: 'Logistics',        route: '/workspace/operations/logistics',   icon: <Truck size={20} />,         color: '#c084fc' },
  maintenance:     { label: 'Maintenance',      route: '/workspace/operations/maintenance', icon: <Wrench size={20} />,        color: '#c084fc' },
  field_ops:       { label: 'Field Ops',        route: '/workspace/operations/field-ops',   icon: <Map size={20} />,           color: '#c084fc' },
  chats:           { label: 'Chats',            route: '/workspace/chats',                  icon: <MessageSquare size={20} />, color: '#38bdf8' },
  email:           { label: 'Email',            route: '/workspace/email',                  icon: <MessageSquare size={20} />, color: '#38bdf8' },
  internals:       { label: 'Internals',        route: '/settings',                         icon: <Settings size={20} />,      color: '#a3a3a3' },
  cog:             { label: 'System Core',      route: '/settings',                         icon: <Cog size={20} />,           color: '#a3a3a3' },
};
...
const DEPARTMENTS: DepartmentDefinition[] = [
  {
    ...
    workspaces: [
      ...
      { key: 'budget', label: 'Budget', icon: <PieIcon size={20} />, route: '/workspaces/finance/budgets', color: '#00f5a0' },
  ...
  },
  {
    ...
    workspaces: [
      { key: 'assets', label: 'Assets', icon: <Briefcase size={20} />, route: '/workspace/inventory/assets', color: '#ffb703' },
      ...
      { key: 'warehouse', label: 'Warehouse', icon: <Building size={20} />, route: '/workspace/inventory/warehouses', color: '#ffb703' },
      { key: 'stock', label: 'Stock', icon: <ClipboardList size={20} />, route: '/workspace/inventory/stock', color: '#ffb703' },
...
function getGreeting() {
  ...
  if (h < 12) return { text: 'Good morning',   icon: <Sun  size={17} className="text-amber-400" /> };
  if (h < 17) return { text: 'Good afternoon', icon: <Sunset size={17} className="text-orange-400" /> };
  return       { text: 'Good evening',   icon: <Moon size={17} className="text-indigo-400" /> };
}
...
const healthData = [
  { name: 'Mon', requests: 4000 }, { name: 'Tue', requests: 3000 },
  { name: 'Wed', requests: 2000 }, { name: 'Thu', requests: 2780 },
  { name: 'Fri', requests: 1890 }, { name: 'Sat', requests: 2390 },
...
const getPermittedOperations = (wsKey: string, permissions: string[], isSuperUser: boolean) => {
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        {dept.workspaces.map((ws) => {
          ...
          const ops = getPermittedOperations(ws.key, currentUser?.permissions || [], isSuperUser);

          return (
            <div
              key={ws.key}
              className={`flex items-center justify-between p-4 rounded-xl border transition-all duration-200 bg-white ${
                permitted ? 'border-gray-150 hover:border-gray-300' : 'border-gray-100 opacity-60'
              }`}
                ...
                <div className="flex flex-col items-start leading-tight">
                  ...
                  {permitted ? (
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {ops.map(op => (
                        <span key={op} className="text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-slate-100 text-slate-500">
                          {op}
                        </span>
                      ))}
                    </div>
...
export default function UserHomeDashboard() {
  ...
  const todayTasks     = tasks.filter(t => isDueToday(t) && t.status !== 'completed' && t.status !== 'cancelled');
  const upcomingTasks  = tasks.filter(isUpcoming);

  const deptData = departments.length > 0
    ? departments.map(d => ({ name: d.name, value: Math.floor(Math.random() * 20) + 5 }))
    : [{ name: 'Engineering', value: 40 }, { name: 'Sales', value: 30 }, { name: 'Marketing', value: 20 }, { name: 'HR', value: 10 }];

  const workspaceBarData = workspaces.length > 0
    ? workspaces.map(w => ({ name: w.name || w.industry_vertical, active: 1 }))
    : [{ name: 'CRM', active: 1 }, { name: 'HR', active: 1 }, { name: 'Finance', active: 1 }];

  const handleToggleTask = async (task: Task) => {
    setActionId(task.id);
    const next = task.status === 'completed' ? 'in_progress' : 'completed';
    try { await authFetch(`/tasks/${task.id}`, { method: 'PUT', body: JSON.stringify({ status: next }) }); await loadTasks(); }
    catch { /* silent */ }
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-[1300px] mx-auto pb-10 animate-[fadeIn_0.25s_ease]">
      ...
      <div className="bg-gradient-to-r from-[#0F2E59] to-[#1a3f73] border border-[var(--border-color)] rounded-2xl py-7 px-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5 shadow-md relative overflow-hidden">
        ...
        <div className="z-10">
          <div className="flex items-center gap-2 mb-1">
            {greeting.icon}
            <span className="text-white/70 text-[0.85rem] font-medium">{greeting.text}</span>
          </div>
          <h1 className="text-[1.65rem] font-black text-white font-display leading-tight">{userName}</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Pending',     value: pendingTasks.length,    color: 'text-amber-400',  bg: 'bg-amber-500/8 border-amber-500/20',    icon: <Clock size={18} /> },
          { label: 'In Progress', value: inProgressTasks.length, color: 'text-sky-400',    bg: 'bg-sky-500/8 border-sky-500/20',        icon: <Zap size={18} /> },
          { label: 'Due Today',   value: todayTasks.length,      color: 'text-accent-primary', bg: 'bg-accent-primary/8 border-accent-primary/20', icon: <CalendarDays size={18} /> },
          { label: 'Overdue',     value: overdueTasks.length,    color: 'text-red-400',    bg: 'bg-red-500/8 border-red-500/20',        icon: <AlertTriangle size={18} /> },
        ].map(c => (
          <div key={c.label} className={`p-5 rounded-xl border flex items-center justify-between ${c.bg}`}>
            <div>
              <p className="text-[0.62rem] font-bold uppercase tracking-wider text-text-muted mb-1">{c.label}</p>
              <p className={`text-2xl font-black ${c.color}`}>{tasksLoading ? '—' : c.value}</p>
            </div>
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            { label: 'Total Personnel',    value: adminLoading ? '—' : users.length,       color: '#00f2fe', icon: <Users size={20} />,      sub: 'Registered IAM Users' },
            { label: 'Active Departments', value: adminLoading ? '—' : departments.length,  color: '#00f5a0', icon: <Layers size={20} />,     sub: 'Operational divisions' },
            { label: 'Cluster Workspaces', value: adminLoading ? '—' : workspaces.length,   color: '#ffb703', icon: <LayoutGrid size={20} />, sub: 'Provisioned & running' },
          ].map(c => (
            <div key={c.label} className="glass-panel p-6 bg-card border border-color rounded-2xl flex justify-between items-start">
              <div>
                <p className="text-[0.72rem] uppercase tracking-wider text-text-muted font-bold mb-4">{c.label}</p>
                <p className="text-[2.2rem] font-extrabold font-display text-text-main leading-none">{c.value}</p>
                <p className="text-[0.78rem] text-text-muted mt-1.5">{c.sub}</p>
              </div>
              <span className="p-2 rounded-lg border border-color bg-card-hover" style={{ color: c.color }}>{c.icon}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="glass-panel p-6 bg-card border border-color rounded-2xl">
              <h3 className="text-[0.95rem] font-bold text-text-main mb-5 flex items-center gap-2">
                <Zap size={17} className="text-[#00f2fe]" /> System Activity & Health
              </h3>
              <div className="w-full h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={healthData} margin={{ top: 5, right: 20, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="uhdGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%"  stopColor="#9d4edd" stopOpacity={0.7} />
                        <stop offset="95%" stopColor="#9d4edd" stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <XAxis dataKey="name" stroke="var(--text-muted)" fontSize={11} tickLine={false} axisLine={false} />
                    <YAxis stroke="var(--text-muted)" fontSize={11} tickLine={false} axisLine={false} />
                    <Tooltip contentStyle={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', borderRadius: '8px', color: 'var(--text-main)', fontSize: 12 }} />
                    <Area type="monotone" dataKey="requests" stroke="#9d4edd" fillOpacity={1} fill="url(#uhdGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="glass-panel p-5 bg-card border border-color rounded-2xl">
                <h3 className="text-[0.85rem] font-bold text-text-main mb-4 flex items-center gap-2">
                  <Briefcase size={15} className="text-[#ffb703]" /> Department Spread
                </h3>
                <div className="w-full h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={deptData} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={4} dataKey="value" stroke="none">
                        {deptData.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                      </Pie>
                      <Tooltip contentStyle={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', borderRadius: '8px', fontSize: 12 }} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="glass-panel p-5 bg-card border border-color rounded-2xl">
                <h3 className="text-[0.85rem] font-bold text-text-main mb-4 flex items-center gap-2">
                  <Server size={15} className="text-[#00f5a0]" /> Workspace Provisioning
                </h3>
                <div className="w-full h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={workspaceBarData} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" vertical={false} />
                      <XAxis dataKey="name" stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} />
                      <Tooltip contentStyle={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', borderRadius: '8px', fontSize: 12 }} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
                      <Bar dataKey="active" fill="#00f5a0" radius={[4, 4, 0, 0]} barSize={22} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="glass-panel bg-card border border-color rounded-2xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-color bg-card-hover">
              ...
              <span className="text-[0.85rem] font-bold text-text-main uppercase tracking-wider">Admin Shortcuts</span>
            </div>
            <div className="flex flex-col gap-3 p-4">
              {[
                { label: 'User Console',       route: '/users',       color: '#9d4edd', icon: <Users size={18} /> },
                { label: 'Departments & Mgmt', route: '/departments', color: '#00f2fe', icon: <Layers size={18} /> },
                { label: 'All Workspaces',     route: '/workspaces',  color: '#00f5a0', icon: <LayoutGrid size={18} /> },
              ].map(s => (
                <button
                  key={s.route}
                  onClick={() => navigate(s.route)}
                  className="flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all hover:brightness-110"
                  style={{ borderColor: `${s.color}30`, backgroundColor: `${s.color}08`, color: 'var(--text-main)' }}
                >
                  <div className="flex items-center gap-3">
                    <span style={{ color: s.color }}>{s.icon}</span>
                    <span className="font-semibold text-[0.85rem]">{s.label}</span>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card border border-color rounded-2xl overflow-hidden shadow-sm">
          <div className="flex justify-between items-center px-5 py-4 border-b border-color bg-card-hover">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-text-main text-[0.82rem] uppercase tracking-wider">Today's Tasks</span>
              {canReadTasks && todayTasks.length > 0 && (
                <span className="text-[10px] font-bold bg-accent-primary text-white px-2 py-0.5 rounded-full">{todayTasks.length}</span>
              )}
            </div>
            {canCreateTasks && (
              <button onClick={() => { setSelectedTask(null); setTaskModalOpen(true); }}
          ...
          <div className="flex flex-col divide-y divide-[var(--border-color)]">
            {!canReadTasks ? (
              <div className="flex flex-col items-center justify-center py-12 text-text-muted gap-2 bg-slate-50/50">
                ...
                <span className="text-sm font-semibold text-text-main">Clearance Required</span>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-text-muted gap-2">
                ...
                <span className="text-sm font-semibold">No tasks due today</span>
              ...
              todayTasks.map(task => {
                ...
                return (
                  <div key={task.id} className={`flex items-center gap-3 px-5 py-3.5 hover:bg-card-hover/40 transition ${done ? 'opacity-60' : ''}`}>
                    <button type="button" onClick={() => handleToggleTask(task)} disabled={actionId === task.id || !canWriteTasks}
                      ...
                      {actionId === task.id
                        ? <Loader2 className="animate-spin text-accent-primary" size={17} />
                        : done ? <CheckSquare className="text-emerald-500" size={17} /> : <Square size={17} />}
                    </button>
                    <div className="flex-1 min-w-0">
                      <p className={`text-[0.83rem] font-semibold text-text-main truncate ${done ? 'line-through text-text-muted' : ''}`}>{task.title}</p>
                      {task.description && <p className="text-[11px] text-text-muted truncate mt-0.5">{task.description}</p>}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ${priorityColor(task.priority)}`}>{task.priority}</span>
                      {canWriteTasks && (
                        <button onClick={() => { setSelectedTask(task); setTaskModalOpen(true); }}
                          className="text-[0.7rem] text-text-muted hover:text-accent-primary cursor-pointer font-semibold">Edit</button>
                      )}
                    </div>
                  </div>
                );
        ...
        <div className="bg-card border border-color rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-color bg-card-hover">
            ...
            <span className="font-bold text-text-main text-[0.82rem] uppercase tracking-wider">Upcoming (7 days)</span>
          </div>
          <div className="flex flex-col divide-y divide-[var(--border-color)]">
            {!canReadTasks ? (
              <div className="flex flex-col items-center justify-center py-12 text-text-muted gap-2 bg-slate-50/50">
                <Lock size={22} className="text-slate-400 opacity-60" />
                <span className="text-xs font-semibold text-text-main">Clearance Required</span>
              </div>
            ) : tasksLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-text-muted gap-2">
                ...
                <span className="text-xs font-semibold">No upcoming deadlines</span>
              </div>
            ) : (
              upcomingTasks.slice(0, 8).map(task => (
                <div key={task.id} className="flex items-center gap-3 px-5 py-3 hover:bg-card-hover/40 transition">
                  <div className="w-1.5 h-1.5 rounded-full bg-accent-primary flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-[0.78rem] font-semibold text-text-main truncate">{task.title}</p>
                    <p className="text-[10px] text-text-muted mt-0.5">
                      {task.due_date ? new Date(task.due_date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }) : ''}
                    </p>
                  </div>
                  <span className={`text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded border ${priorityColor(task.priority)}`}>{task.priority[0]}</span>
                </div>
              ))
            )}
          </div>
      ...
      {(() => {
        ...
        const isDeptVisible = (dept: typeof DEPARTMENTS[0]) => {
          ...
          if (userDeptId && (
            ...
            (dept.id === 'crm' && (userDeptId.includes('crm') || userDeptId.includes('sales'))) ||
            (dept.id === 'operations' && userDeptId.includes('operations')) ||
            (dept.id === 'hr' && (userDeptId.includes('hr') || userDeptId.includes('company'))) ||
            (dept.id === 'communications' && userDeptId.includes('communication')) ||
            (dept.id === 'utilities' && userDeptId.includes('utilit')) ||
            (dept.id === 'internals' && userDeptId.includes('internal'))
          )) {
            return true;
          }
          return dept.workspaces.some(ws => permittedWorkspaceKeys.includes(ws.key));
        };
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[1.1rem] font-bold text-text-main font-display">
                {isAdmin ? 'Departmental Workspaces' : 'My Workplace Department'}
              </h2>
            </div>
      ...
      <CreateTaskModal
        ...
        onSuccess={() => { setTaskModalOpen(false); loadTasks(); }}
