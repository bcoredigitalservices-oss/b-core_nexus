    ...
    try {
      ...
      if (!projRes.ok || !taskRes.ok) {
        if (projRes.status === 403 || taskRes.status === 403) {
          throw new Error('Access Denied: You do not have permission to view the Operations Workspace.');
        }
      ...
      const activeProj = projectsList.filter(p => p.status === 'ACTIVE').length;
      setActiveProjectsCount(activeProj);

      // 2. Tasks Overdue/Outstanding (non-DONE tasks, especially high/urgent priority)
      const overdueTasks = tasksList.filter(t => t.status !== 'DONE' && (t.priority === 'URGENT' || t.priority === 'HIGH')).length;
      ...
      setErrorMsg(err.message || 'An error occurred while fetching operations data.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem', width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '0.2rem' }}>
              Operations control center. Coordinate project timelines, task prioritization, and operational milestones.
            </p>
          </div>
          <button
        ...
        <div style={{
          ...
          >
            ...
            <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: '#f59e0b' }}>
                        ...
                        <tr key={tsk.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                          ...
                          <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                            <span style={{
                              ...
                              fontSize: '0.7rem',
                              fontWeight: 700,
                              background: tsk.status === 'DONE' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(255, 255, 255, 0.05)',
                              color: tsk.status === 'DONE' ? '#10b981' : 'var(--text-muted)'
                            }}>
                              {tsk.status}
                            </span>
                          </td>
