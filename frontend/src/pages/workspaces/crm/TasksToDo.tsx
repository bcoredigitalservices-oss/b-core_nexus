...
export default function TasksToDo() {
  ...
  const canReadTasks = currentUser?.permissions?.includes('*:*') ||
  ...
  const canCreateTasks = currentUser?.permissions?.includes('*:*') ||
                         currentUser?.permissions?.includes('tasks:create') ||
                         currentUser?.functional_roles?.includes('admin') ||
                         currentUser?.functional_roles?.includes('manager');

  const canWriteTasks = currentUser?.permissions?.includes('*:*') ||
  ...
  const getAssigneeName = (ownerId: string) => {
    ...
    return matched.first_name ? `${matched.first_name} ${matched.last_name || ''}` : matched.email;
  };
  ...
  const filteredTasks = tasks.filter((tsk) => {
    if (searchTitle.trim()) {
      ...
      const matchDesc = tsk.description ? tsk.description.toLowerCase().includes(q) : false;
      if (!matchTitle && !matchDesc) return false;
    }
    if (filterStatus !== 'all' && tsk.status !== filterStatus) return false;
    if (filterPriority !== 'all' && tsk.priority !== filterPriority) return false;
    if (filterAssignee !== 'all' && tsk.owner_id !== filterAssignee) return false;
    return true;
  });
  ...
  if (!canReadTasks) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[450px] text-text-muted gap-4">
        ...
        <span className="text-sm">You do not have clearance to view operational tasks. (Requires tasks:read permission)</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-8 w-full max-w-[1250px] mx-auto p-4 relative animate-[fadeIn_0.3s_ease]">
        ...
        <div className="flex items-center gap-4.5 z-10">
          ...
          <div>
            ...
            <p className="text-white/80 text-[0.82rem] font-medium max-w-[550px] leading-relaxed">
              Create, schedule, and assign core system checklists and tasks to operator profiles, monitoring compliance in real time.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-panel p-5 flex items-center justify-between bg-card border border-color rounded-xl shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col gap-1">
            <span className="text-[0.7rem] text-text-muted font-bold uppercase tracking-wider">Total Tasks</span>
            <span className="text-2xl font-black text-text-main leading-none">{totalCount}</span>
            <span className="text-[10px] text-text-muted mt-1 font-semibold">Active in ledger</span>
          </div>
        ...
        <div className="glass-panel p-5 flex items-center justify-between bg-card border border-color rounded-xl shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col gap-1">
            <span className="text-[0.7rem] text-text-muted font-bold uppercase tracking-wider">Pending</span>
            <span className="text-2xl font-black text-text-main leading-none">{pendingCount}</span>
            <span className="text-[10px] text-amber-500 font-semibold mt-1">Pending Start</span>
          </div>
        ...
        <div className="glass-panel p-5 flex items-center justify-between bg-card border border-color rounded-xl shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col gap-1">
            <span className="text-[0.7rem] text-text-muted font-bold uppercase tracking-wider">In Progress</span>
            <span className="text-2xl font-black text-text-main leading-none">{inProgressCount}</span>
            <span className="text-[10px] text-sky-500 font-semibold mt-1">Being Executed</span>
          </div>
        ...
        <div className="glass-panel p-5 flex items-center justify-between bg-card border border-color rounded-xl shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col gap-1">
            <span className="text-[0.7rem] text-text-muted font-bold uppercase tracking-wider">Completed</span>
            <span className="text-2xl font-black text-text-main leading-none">{completedCount}</span>
            <span className="text-[10px] text-emerald-500 font-semibold mt-1">Done & Audited</span>
          </div>
      ...
      <div className="glass-panel p-5 bg-card border border-color rounded-xl shadow-sm flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-[0.7rem] text-text-muted font-bold uppercase tracking-wider mb-1.5">Search Query</label>
          ...
          <div>
            <label className="block text-[0.7rem] text-text-muted font-bold uppercase tracking-wider mb-1.5">Status</label>
            <select
              value={filterStatus}
          ...
          <div>
            <label className="block text-[0.7rem] text-text-muted font-bold uppercase tracking-wider mb-1.5">Priority</label>
            <select
          ...
          <div>
            <label className="block text-[0.7rem] text-text-muted font-bold uppercase tracking-wider mb-1.5">Assignee</label>
            <select
            ...
            >
              ...
              {users.map((op) => (
                <option key={op.id} value={op.id}>
                  {op.first_name ? `${op.first_name} ${op.last_name || ''}` : op.email}
                </option>
              ))}
            </select>
          </div>
        ...
        {(searchTitle || filterStatus !== 'all' || filterPriority !== 'all' || filterAssignee !== 'all') && (
      ...
      <div className="glass-panel p-0 overflow-hidden bg-card border border-color rounded-2xl shadow-sm">
        <div className="py-5 px-7 border-b border-color flex items-center justify-between bg-card-hover">
          <span className="text-[0.8rem] font-bold text-text-main tracking-wider uppercase">Tasks Roster</span>
            ...
            <tbody>
              {filteredTasks.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-text-muted">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <span className="text-sm font-semibold">No operational tasks matches your criteria.</span>
                      {(searchTitle || filterStatus !== 'all' || filterPriority !== 'all' || filterAssignee !== 'all') && (
                        <button 
                          onClick={handleClearFilters}
                ...
                filteredTasks.map((tsk) => {
                  ...
                  return (
                    <tr key={tsk.id} className={`border-b border-color transition-colors duration-150 hover:bg-card-hover/30 ${isCompleted ? 'opacity-80' : ''}`}>
                      ...
                      <td className="py-4 px-6 text-center">
                        <button
                          ...
                          disabled={actionLoadingId === tsk.id || !canWriteTasks}
                          ...
                          title={!canWriteTasks ? "Requires task writing clearance" : isCompleted ? "Mark in progress" : "Mark completed"}
                        >
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col">
                          <span className={`font-semibold text-text-main text-[0.88rem] ${isCompleted ? 'line-through text-text-muted' : ''}`}>
                            {tsk.title}
                          </span>
                          {tsk.description && (
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-text-main text-[0.82rem] truncate max-w-[140px]" title={getAssigneeName(tsk.owner_id)}>
                      ...
                      <td className="py-4 px-4 font-medium text-[0.82rem] text-text-muted flex items-center gap-1.5 mt-1.5">
                        ...
                        {tsk.due_date ? new Date(tsk.due_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : <em className="opacity-40">No due date</em>}
                      </td>
