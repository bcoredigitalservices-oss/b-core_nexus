...
import { Target, Plus, Filter, PieChart, X } from 'lucide-react';
import UniversalDataGrid from '../../../components/ui/UniversalDataGrid';

export default function Budget() {
  const [activeTab, setActiveTab] = useState<'budgets' | 'variance'>('budgets');
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  const accentColor = activeTab === 'budgets' ? '#059669' : '#0ea5e9'; // Emerald and Sky Blue
      ...
      >
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0, color: 'var(--text-main)' }}>
            {activeTab === 'budgets' ? 'Create New Budget' : 'Generate Variance Report'}
          </h3>
          ...
          {activeTab === 'budgets' ? (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Budget Name</label>
                <input type="text" placeholder="e.g. Q3 Marketing" style={{ padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-main)' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Total Amount</label>
                <input type="number" placeholder="100000" style={{ padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-main)' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Fiscal Year</label>
            ...
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Select Budget</label>
                <select style={{ padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-main)' }}>
                  <option>Q3 Marketing</option>
                  <option>Annual IT</option>
                </select>
              </div>
            </>
          )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', opacity: isDrawerOpen ? 0.8 : 1, transition: 'opacity 0.2s' }}>
        ...
        >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '0.4rem' }}>
                ...
                <span 
                  style={{ 
                    ...
                    background: `linear-gradient(135deg, ${accentColor}, ${activeTab === 'budgets' ? '#047857' : '#0369a1'})`,
                    color: '#fff',
                    padding: '4px 12px',
                    borderRadius: '20px',
                    boxShadow: `0 4px 12px ${accentColor}40`
                ...
                >
                  {activeTab === 'budgets' ? 'Cost Centers' : 'Analytics'}
                </span>
              </div>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', margin: 0, fontWeight: 500 }}>
                Set organizational budgets and monitor variance in real-time.
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1rem', zIndex: 1 }}>
            ...
            >
              <Plus size={18} /> {activeTab === 'budgets' ? 'Create Budget' : 'Run Variance Report'}
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', background: 'var(--bg-card)', padding: '0.5rem', borderRadius: '12px', border: '1px solid var(--border-color)', boxShadow: '0 2px 8px rgba(0,0,0,0.02)', width: 'max-content' }}>
          <button
            ...
            style={{
              ...
              background: activeTab === 'budgets' ? '#05966915' : 'transparent',
              color: activeTab === 'budgets' ? '#059669' : 'var(--text-muted)',
              border: 'none', borderRadius: '8px', fontWeight: 700, fontSize: '0.85rem',
              cursor: 'pointer', transition: 'all 0.2s'
            }}
          >
            <Target size={16} /> Budget Master
          </button>
          <button
            onClick={() => setActiveTab('variance')}
            style={{
              display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px',
              background: activeTab === 'variance' ? '#0ea5e915' : 'transparent',
              color: activeTab === 'variance' ? '#0ea5e9' : 'var(--text-muted)',
              border: 'none', borderRadius: '8px', fontWeight: 700, fontSize: '0.85rem',
              cursor: 'pointer', transition: 'all 0.2s'
            }}
          >
            <PieChart size={16} /> Variance Tracking
          </button>
        </div>
        ...
        <div style={{ flex: 1, backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
          <UniversalDataGrid 
            endpointUrl={`/api/v1/workspaces/finance/budgets?type=${activeTab}`} 
            title=""
            columns={
              activeTab === 'budgets' ? [
                ...
                { key: 'allocated_amount', label: 'Allocated', sortable: true },
                ...
                { key: 'budget_name', label: 'Budget Name', sortable: true },
                { key: 'allocated_amount', label: 'Allocated', sortable: true },
                { key: 'actual_amount', label: 'Actual Spent', sortable: true },
                { key: 'variance', label: 'Variance', sortable: true },
                { key: 'variance_percent', label: 'Variance %', sortable: true },
