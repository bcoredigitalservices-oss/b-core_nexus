...
export function CreateDealModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // owner_id is never part of this schema — it's server-assigned on create.
      const created = await authFetch("/crm/deals", {
        method: "POST",
        body: JSON.stringify({
      ...
      setErrorMsg(err.message || "Failed to create deal opportunity.");
    } finally {
      setSaving(false);
    }
  };
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value) setLeadId(""); // mutually exclusive link for simplicity
                }}
                className="w-full rounded-lg border border-color bg-main py-2 px-3 text-xs outline-none focus:border-accent-primary text-[var(--text-main)] cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value) setCustomerId(""); // mutually exclusive link
                }}
                className="w-full rounded-lg border border-color bg-main py-2 px-3 text-xs outline-none focus:border-accent-primary text-[var(--text-main)] cursor-pointer"
              >
