...
export default function Login() {
 ...
 /* ── UPDATED: TOTP verification submit ─────────────────────────────────── */
  const handleTotpSubmit = async (e: FormEvent) => {
    ...
    const finalEmail = email.trim() || emailRef.current?.value.trim() || '';
    const finalPassword = password || passwordRef.current?.value || '';

    try {
      const body = new URLSearchParams();
      body.append('username', finalEmail);
      body.append('password', finalPassword);
      body.append('grant_type', 'password'); // Required by your schema pattern
      body.append('totp_code', totpCode.trim());
