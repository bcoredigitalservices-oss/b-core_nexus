...
export default function UsersDashboard() {
  ...
  const canManageRoles =
    isUserAdmin(currentUser) || hasPermission(currentUser, "iam:manage");
                        ...
                        <tr key={user.id} className="border-b border-color transition-colors duration-150 hover:bg-card-hover/40">
                          ...
                          <td className="py-4 px-6 text-right flex justify-end items-center gap-2">
                            <button
                              ...
                              className="py-1.5 px-3.5 bg-card hover:bg-card-hover border border-color text-text-main text-[0.78rem] font-bold rounded-lg cursor-pointer transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                              Configure Access
                            </button>
                            {user.is_active ? (
