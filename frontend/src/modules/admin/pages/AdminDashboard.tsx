    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [orgRes, usersRes, deptsRes, worksRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL}/api/v1/organization/profile`, {
            headers,
          }),
          fetch(`${import.meta.env.VITE_API_URL}/api/v1/auth/users`, {
            headers,
          }),
          fetch(`${import.meta.env.VITE_API_URL}/api/v1/iam/departments`, {
            headers,
          }),
          fetch(`${import.meta.env.VITE_API_URL}/api/v1/iam/workspaces`, {
            headers,
          }),
        ]);
    ...
    <div className="flex flex-col gap-8 w-full max-w-[1400px] mx-auto pb-8">
      ...
      <div className="bg-gradient-to-br from-[#9d4edd]/8 to-[#00f2fe]/3 border border-color rounded-2xl py-8 px-10 flex justify-between items-center flex-wrap gap-6 relative overflow-hidden shadow-lg">
        <div
          ...
          style={{
            background:
              "radial-gradient(circle, rgba(157, 78, 221, 0.15) 0%, rgba(0,0,0,0) 70%)",
          }}
        />
        ...
        <div className="flex items-center gap-6 z-10">
          ...
          <div>
            ...
            <p className="text-text-muted text-[0.9rem] max-w-[600px] leading-relaxed">
              Welcome back, {currentUser?.username || "Executive"}. Here is the
              live operating status of your organization, including department
              health and active workspaces.
            </p>
          </div>
        </div>
